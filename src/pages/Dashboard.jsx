...
export function Dashboard() {
  ...
  if (loading) return <div className="min-h-screen bg-background flex items-center justify-center font-mono text-muted-foreground text-sm">Initializing Intelligence...</div>;

  const a = REAL_P4P_ROSTER[0];
  const b = REAL_P4P_ROSTER[1];

  return (
    <div className="min-h-screen bg-background text-foreground font-sans pb-10 md:pb-20 transition-colors duration-300">
      ...
      <section className="relative pt-20 pb-16 md:pt-28 md:pb-32 border-b border-border overflow-hidden">
        ...
        <div className="container relative z-20 px-4 md:px-8">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-surface/80 backdrop-blur-sm px-3 py-1.5 text-xs font-medium mb-4 md:mb-6 shadow-sm">
            ...
            <p className="mt-4 md:mt-6 text-base md:text-lg font-medium text-foreground/80 max-w-xl">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center gap-3">
              ...
              <Link to="/fighters" className="w-full sm:w-auto inline-flex items-center justify-center h-12 px-8 rounded-md bg-surface border border-border text-foreground font-bold hover:bg-secondary transition-colors shadow-sm">
      ...
      <section className="container py-12 md:py-16 px-4 md:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 mb-6">
          <div>
            <div className="text-[10px] md:text-xs font-bold tracking-[0.18em] uppercase text-muted-foreground">Featured Matchup</div>
            <h2 className="mt-1 text-2xl md:text-3xl font-bold tracking-tight">{a.name} vs {b.name}</h2>
          </div>
          ...
          <div className="grid lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-border">
            ...
            <div className="p-5 md:p-8 grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-6 bg-surface-2 relative">
              ...
              <div className="relative text-center px-2">
                <div className="text-[9px] md:text-[10px] font-bold tracking-[0.3em] text-muted-foreground">VS</div>
                <div className="mt-1 md:mt-2 stat-num text-sm md:text-xl font-black">Moderate</div>
                <div className="text-[9px] md:text-[10px] font-bold tracking-widest text-muted-foreground uppercase">Edge</div>
              </div>
              <FeaturedFighter fighter={b} side="right" pct={38} />
            </div>
            <div className="p-6 md:p-8 flex flex-col justify-center">
              <div className="text-[10px] md:text-xs font-bold tracking-[0.18em] uppercase text-muted-foreground flex items-center gap-2 mb-2 md:mb-3">
                <span className="text-primary"><Icons.Activity /></span> Forecast
              </div>
              <p className="text-sm md:text-base font-medium text-foreground/90">Based on historical database records, {a.name} carries the statistical edge.</p>
              <div className="mt-4 md:mt-6 flex h-2 md:h-3 rounded-full overflow-hidden bg-secondary border border-border">
              ...
              <div className="mt-2 flex items-center justify-between font-bold">
                <div><span className="stat-num text-xl md:text-2xl text-primary">62%</span><span className="text-[10px] md:text-xs text-muted-foreground ml-2">{a.name.split(" ").slice(-1)[0]}</span></div>
                <div className="text-right"><span className="text-[10px] md:text-xs text-muted-foreground mr-2">{b.name.split(" ").slice(-1)[0]}</span><span className="stat-num text-xl md:text-2xl text-blue-500">38%</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="container py-12 md:py-16 px-4 md:px-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 mb-6">
          <div>
            <div className="text-[10px] md:text-xs font-bold tracking-[0.18em] uppercase text-muted-foreground">UFC Official · Pound-for-pound</div>
            <h2 className="mt-1 text-2xl md:text-3xl font-bold tracking-tight">Men's P4P · Top rankings</h2>
          </div>
      ...
      <section className="container py-12 md:py-16 px-4 md:px-8">
        <div className="flex items-end justify-between mb-6">
          <div>
            <div className="text-[10px] md:text-xs font-bold tracking-[0.18em] uppercase text-muted-foreground">Trending</div>
            <h2 className="mt-1 text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2"><div className="text-primary"><Icons.TrendingUp /></div> High Output Fighters</h2>
          </div>
        </div>
        <div className="surface-card overflow-hidden">
          <div className="divide-y divide-border">
            {TRENDING_ROSTER.map((f, i) => (
              <Link key={f.id} to={`/fighters/${f.id}`} className="grid grid-cols-[30px_1fr_auto] sm:grid-cols-[40px_1fr_120px_140px_120px] items-center gap-3 md:gap-4 px-4 md:px-5 py-3 md:py-4 hover:bg-surface-2 transition-colors">
                <div className="stat-num text-lg md:text-xl text-muted-foreground">{String(i + 1).padStart(2, "0")}</div>
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] md:text-xs font-medium text-muted-foreground truncate">{f.weightClass}</div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  <div className="stat-num text-xs md:text-sm font-bold">{f.slpm}</div>
                  <div className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-muted-foreground">SLpM</div>
                </div>
              </Link>
            ))}
          </div>
      ...
      <section className="container py-12 md:py-20 px-4 md:px-8">
        <div className="surface-card relative overflow-hidden p-8 md:p-14 text-center border-primary/20">
          <div className="relative z-10">
            <h2 className="text-3xl md:text-5xl font-extrabold tracking-tighter">Predict the next title fight.</h2>
            <p className="mt-3 md:mt-4 text-sm md:text-base text-muted-foreground font-medium max-w-xl mx-auto">Run any matchup through our analytical engine. Strikes, takedowns, defense, form — distilled into a single edge.</p>
            <Link to="/compare" className="mt-6 md:mt-8 inline-flex items-center justify-center w-full sm:w-auto h-12 px-8 rounded-md bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-all shadow-md">
...
function StatBlock({ label, value, icon }) {
  return (
    <div className="bg-surface/80 p-4 md:p-5 flex flex-col items-center md:items-start text-center md:text-left backdrop-blur-sm">
      <div className="text-primary mx-auto md:mx-0">{icon}</div>
      <div className="mt-2 md:mt-3 stat-num text-2xl md:text-3xl">{value}</div>
      <div className="mt-1 text-[9px] md:text-xs font-bold text-muted-foreground tracking-wider uppercase">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className={`relative flex ${side === "right" ? "flex-row-reverse text-right" : ""} items-center gap-3 md:gap-4`}>
      ...
      <div className="min-w-0">
        ...
        <div className="text-sm sm:text-lg md:text-xl font-extrabold tracking-tight truncate">{fighter.name}</div>
        <div className="hidden sm:block stat-num text-xs md:text-sm font-bold text-muted-foreground">{fighter.w}-{fighter.l}-0</div>
        <div className={`mt-1 md:mt-2 stat-num text-lg md:text-2xl ${side === "left" ? "text-primary" : "text-blue-500"}`}>{pct}%</div>
      </div>
    </div>
  );
}
    ...
    <div className="group relative bg-surface border border-border rounded-xl overflow-hidden hover:-translate-y-1 hover:shadow-2xl hover:border-primary/30 transition-all duration-300 cursor-pointer block">
      ...
      <div className="p-4 md:p-5">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-muted-foreground">
              <span className="truncate">{fighter.weightClass}</span>
            </div>
            <h3 className="mt-1 text-base md:text-lg font-extrabold tracking-tight truncate group-hover:text-primary transition-colors">{fighter.name}</h3>
          </div>
          <div className="text-right shrink-0">
            <div className="stat-num text-base md:text-lg leading-none">{fighter.w}-{fighter.l}</div>
            <div className="text-[9px] md:text-[10px] font-bold text-muted-foreground mt-1 tracking-wider uppercase">Record</div>
          </div>
        </div>

        <div className="mt-4 md:mt-5 grid grid-cols-2 gap-2 text-center">
          <div className="rounded-md bg-secondary border border-border py-2">
            <div className="stat-num text-xs md:text-sm">{fighter.strAcc}%</div>
            <div className="text-[8px] md:text-[9px] font-bold text-muted-foreground tracking-widest uppercase mt-0.5">STR ACC</div>
          </div>
          <div className="rounded-md bg-secondary border border-border py-2">
            <div className="stat-num text-xs md:text-sm">{fighter.tdDef}%</div>
            <div className="text-[8px] md:text-[9px] font-bold text-muted-foreground tracking-widest uppercase mt-0.5">TD DEF</div>
          </div>
        </div>
      </div>
      <div className="px-4 md:px-5 pb-4 md:pb-5">
        <Link to={`/compare?a=${fighter.id}`} className="inline-flex items-center justify-center w-full h-9 md:h-10 rounded-md bg-secondary border border-border text-foreground font-bold text-sm hover:bg-surface-3 transition-colors shadow-sm">
